...
- The sitemap lists the home page only, dated with the build, and nothing schedules a build. The
  JSON-LD describes the site, the page and the client, and invents no rating or review.
- `/sitemap.xml` is an alias generated by `src/pages/sitemap.xml.ts`: a sitemap index pointing at the
  integration's `sitemap-0.xml`, since `@astrojs/sitemap` always names its own index
  `sitemap-index.xml`. It lists no URL of its own. Never hand-write a list of URLs into either.
